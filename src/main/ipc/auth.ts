...
export function registerAuthHandlers(): void {
  ...
  ipcMain.handle(AuthChannels.EMPLOYEE_LOGIN, async (_, { email, password }: { email: string; password: string }) => {
    try {
      ...
      return { success: true, user: { id: user.id, email: user.email, role: user.role as 'TEACHER' } }
    } catch (err) {
      console.error('[auth/employee-login]', err)
      return { success: false, error: (err as Error).message }
    }
  })

  ipcMain.handle(AuthChannels.EMPLOYEE_SIGNUP, async (_, { email, password }: { email: string; password: string }) => {
    try {
      ...
      console.error('[auth/employee-signup]', err)
      return { success: false, error: (err as Error).message }
    }
  })
